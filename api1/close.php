...
if ($data !== null) {
    ...
    $sql = "SELECT * FROM token WHERE correo_electronico = '$correo_electronico' AND estatus = 'abierta'";
    $result = $conexion->query($sql);

    // Verificar si se encontraron resultados
    if ($result->num_rows > 0) {
        // Cambiar el estatus del token a "cerrada"
        $updateTokenSQL = "UPDATE token SET estatus = 'cerrada' WHERE correo_electronico = '$correo_electronico' AND estatus = 'abierta'";
